...
final initialState = <OfflineOperation>[];
...
void main() {
  ...
  setUpAll(() async {
    ...
    registerFallbackValue(
      OfflineOperation(type: OperationType.updateStock, targetProductId: 'id'),
    );
  });
  ...
  setUp(() {
    ...
    // Initialize container with necessary services mocked
  ...
  group('OfflineQueueService Unit Test', () {
    test(
      'enqueue adds an operation to the Hive box and notifies listeners',
      () async {
        final listener = Listener<List<OfflineOperation>>();

        // Override the service provider with mocked dependencies
        container.updateOverrides([
          offlineQueueServiceProvider.overrideWith(
            (ref) => OfflineQueueService(
              initialState, // List<OfflineOperation>
              mockQueueBox, // Box<OfflineOperation>
        ...
        await container
            .read(offlineQueueServiceProvider.notifier)
            .enqueue(operation);
  ...
  group('ProductTile Widget Test', () {
    final testProduct = Product(id: 'p001', name: 'Test Product', stock: 10);

    testWidgets('Manager sees stock edit controls', (tester) async {
      // Setup: Manager user (has manager permission, but not admin)
      when(
        () => mockAuthService.hasPermission(any(), AppRole.manager),
      ).thenReturn(true);
      when(
        () => mockAuthService.hasPermission(any(), AppRole.admin),
      ).thenReturn(false);
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          overrides: [
            ...
            inventoryManagerProvider.overrideWith(
              (ref) => InventoryManager(
                ref,
                [testProduct],
                mockProductBox,
                mockBackendService,
                container.read(offlineQueueServiceProvider.notifier),
              ),
            ),
